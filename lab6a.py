...
class Student:
    ...
    def displayGPA(self):
        ...
        gpa = 0.0 
        count_not_zero = 0 # Counter for non-zero grades
        for course in self.courses.keys():
            gpa = gpa + self.courses[course] # Add the grade to GPA
            if self.courses[course] != 0.0: # check if its non zero 
                count_not_zero = count_not_zero + 1 #increment 
        if count_not_zero == 0: 
            return 'GPA of student ' + self.name + ' is ' + '0.0'
